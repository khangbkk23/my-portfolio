      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-14">
        {links.map((link, i) => (
          <motion.a
            ...
            className="flex items-center gap-4 rounded-2xl border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 px-5 py-4 hover:border-violet-300 dark:hover:border-violet-800 transition-colors duration-200 group"
          >
            <span className={`${link.color} group-hover:scale-110 transition-transform`}>{link.icon}</span>
            <div>
              <p className="text-[11px] font-semibold tracking-widest uppercase text-neutral-400 mb-0.5">{link.label}</p>
              <p className="text-sm font-medium text-black dark:text-white">{link.value}</p>
            </div>
          </motion.a>
        ))}
      </div>
