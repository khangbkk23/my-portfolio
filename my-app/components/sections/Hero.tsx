...
export default function Hero() {
    ...
    return (
        <div className="w-full min-h-screen relative flex items-center pt-24 pb-16">
            
            {/* Đốm sáng mờ nhẹ ở chính giữa nền để tạo điểm nhấn mà không làm gãy background */}
            <div className="absolute top-1/4 left-1/2 -translate-x-1/2 w-[500px] h-[500px] md:w-[700px] md:h-[700px] bg-violet-500/10 dark:bg-indigo-500/10 blur-[120px] rounded-full pointer-events-none -z-10" />
            ...
            {mounted && (
                <motion.button
                    ...
                    className="fixed top-10 right-4 sm:right-10 z-50 p-3 rounded-full border border-neutral-200 dark:border-neutral-800 bg-white/50 dark:bg-black/50 backdrop-blur-md text-neutral-800 dark:text-neutral-200"
                >
                    {theme === "dark" ? (
            ...
            <div className="relative z-10 w-full max-w-5xl mx-auto px-6 lg:px-8">
                <div className="hidden lg:grid grid-cols-1 lg:grid-cols-2 gap-x-12 gap-y-12 items-center text-center lg:text-left mb-16 lg:mb-20">
                ...
                <div className="lg:hidden flex flex-col items-center gap-10 text-center px-4 py-8">
                    <motion.div {...fadeUp(0)}>
                        <span className="inline-flex items-center gap-2 text-xs font-medium tracking-widest uppercase text-neutral-400 dark:text-neutral-500">
                            <span className="w-8 h-px bg-neutral-300 dark:bg-neutral-700" />
                            Student Portfolio
                    ...
                    <motion.h1
                        {...fadeUp(0.1)}
                        className="text-3xl sm:text-4xl font-bold text-black dark:text-white leading-tight tracking-tight"
                    >
                        Hi, I&apos;m <span className="text-transparent bg-clip-text bg-gradient-to-r from-violet-600 to-indigo-500 dark:from-violet-400 dark:to-indigo-300">Khang</span>
                        <span className="block text-base sm:text-lg text-neutral-400 dark:text-neutral-500 mt-2">
                            AI Engineer | Data Scientist
                        </span>
                    </motion.h1>
                    ...
                    <motion.p
                        {...fadeUp(0.2)}
                        className="text-sm text-neutral-400 dark:text-neutral-500 flex items-center justify-center gap-2 flex-wrap"
                    >
                        <span className="w-1.5 h-1.5 rounded-full bg-violet-500 animate-pulse" />
                    ...
                    >
                        ...
                        <motion.div
                            animate={{ y: [-12, 12, -12] }}
                            transition={{ repeat: Infinity, duration: 8, ease: "easeInOut" }}
                            className="relative z-10 w-64 h-64 mx-auto sm:w-72 sm:h-72"
                        >
                            <Image
                                src="/avt.jpeg"
                                alt="Khang Profile Picture"
                                fill
                                className="object-contain rounded-[40px] drop-shadow-[0_25px_60px_rgba(99,102,241,0.5)] dark:drop-shadow-[0_25px_60px_rgba(99,102,241,0.3)]"
                                priority
                            />
                        </motion.div>
                    </motion.div>

                    <motion.p {...fadeUp(0.3)} className="text-sm text-neutral-500 max-w-xl leading-relaxed">
                        {profile.shortGoal}
                    </motion.p>

                    <motion.div {...fadeUp(0.4)} className="flex flex-wrap justify-center gap-2 max-w-xs">
                    ...
                    <motion.div {...fadeUp(0.5)} className="flex flex-col gap-3 items-center w-full max-w-sm mx-auto">
                        <motion.a
                            whileHover={{ scale: 1.03 }}
                            whileTap={{ scale: 0.97 }}
                            href={`mailto:${profile.contact.email}`}
                            className="inline-flex w-full justify-center items-center gap-2 px-6 py-3 rounded-full bg-black dark:bg-white text-white dark:text-black text-sm font-semibold shadow-xl shadow-black/10 dark:shadow-white/10"
                        >
                            Work With Me
                        </motion.a>
                        <motion.div className="flex w-full gap-3">
                            <motion.a
                                ...
                                target="_blank"
                                rel="noreferrer"
                                className="inline-flex flex-1 justify-center items-center gap-2 px-4 py-3 rounded-full border border-neutral-200 dark:border-neutral-700 text-sm font-semibold text-neutral-700 dark:text-neutral-300 bg-white/50 dark:bg-black/50 backdrop-blur-sm"
                            >
                                GitHub
                            </motion.a>
                            <motion.a
